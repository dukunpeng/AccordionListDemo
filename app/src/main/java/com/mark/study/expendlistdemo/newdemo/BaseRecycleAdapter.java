...
public abstract class BaseRecycleAdapter<T> extends RecyclerView.Adapter {
    ...
    //构造器list:要加载的数据集合，itemlayout:item的布局（R.layout.item_preview_viewpgaer）,isNeedMore:是否需要加载更多的功能（false:不需要，true:需要）
    ...
    //构造器list:要加载的数据集合，itemlayout:item的布局（R.layout.item_preview_viewpgaer）,isNeedMore:是否需要加载更多的功能（false:不需要，true:需要）
    public BaseRecycleAdapter(Context context, List<T> list, int itemLayout, double h2w, boolean isNeedMore) {
        if (list == null) {
            list = new ArrayList<T>();
        }
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_HEADER) {
            ...
            //返回item的布局的holder  recyclerview这里加载itemview的inflate 写成这种的layoutparam才有效
            View view = LayoutInflater.from(context).inflate(itemLayout, parent, false);
            if (h2w != 0) {
                //实测在8.0华为手机上 如果使用ConstraintLayout 有个app:layout_constraintDimensionRatio属性 会不起作用，
                //所以我手动给设置比例，且8.0上设置比例的话，要给layoutParams.height，layoutParams.width同时设置长度，如果不同时设置，也不起作用，暂时没找见原因
              /*  ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
                layoutParams.height = (int) (ScreenUtil.getInstance(context).getScreenWidth() * h2w);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        //((MyViewHolder) holder).item_preview_viewpgaer.setText(position + "");
            ...
            holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if (onItemLongClick != null) {
                        onItemLongClick.onItemLongClick(v, position);
                    }
                    return false;
                }
            });
    ...
    public  class MyViewHolder extends RecyclerView.ViewHolder {
         ...
         * 使用 Glide 为指定 viewId 的 ImageView 对象设置图片
         */
        public MyViewHolder bindImageAsBitmap(@IdRes int viewId, @NonNull String url) {
            if (getImageView(viewId) != null) {
                Glide.with(context).load(url).asBitmap().into(getImageView(viewId));
            }
            return this;
        }
         ...
         * 使用 Glide 为指定 viewId 的 ImageView 对象设置图片
         */
        public MyViewHolder bindImageAsBitmapWithCache(@IdRes int viewId, @NonNull String url) {
            if (getImageView(viewId) != null) {
                if (!TextUtils.isEmpty(url)) {
                    if (!url.equals(getImageView(viewId).getTag())) {
                        Glide.with(context).load( url).asBitmap().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(getImageView(viewId));
                    }
                }
...
//            if ( imageView!= null) {
//                if (!TextUtils.isEmpty(url)){
//                    if (!tag.equals(imageView.getTag(R.integer.image_tag))){
//                        imageView.setTag(R.integer.image_tag,tag);
